// Package cli assembles the exchangerate command tree from the exchangerate
...
// NewApp assembles the kit application from the exchangerate domain. The
...
//
// To add a command, declare it in exchangerate/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := exchangerate.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(exchangerate.Domain{}).Register(app)
