...
describes what users get. Pick a format with `-o`, or let exchangerate choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
exchangerate <command> -o table   # aligned columns for reading
exchangerate <command> -o jsonl   # one JSON object per line, for piping
exchangerate <command> -o json    # a single JSON array
exchangerate <command> -o csv     # spreadsheet friendly
exchangerate <command> -o tsv     # tab-separated
exchangerate <command> -o url     # just the URL column
exchangerate <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
exchangerate <command> --fields id,title,url
```
...
```bash
exchangerate <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
exchangerate <command>            # a table, because this is a terminal
exchangerate <command> | wc -l    # JSONL, because this is a pipe
```
